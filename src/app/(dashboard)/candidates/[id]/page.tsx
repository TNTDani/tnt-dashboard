...
import { CandidateProfile, TimelineEntry, FollowUp } from '@/lib/types';
...
export default function CandidateDetailPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      db.getFollowUps(),
      db.getCandidates(),
    ]).then(([profiles, vacancyList, allFollowUps, pipelineCandidates]) => {
      ...
      // Load active follow-up for this candidate
  ...
  const handleEmailSent = useCallback((entry: TimelineEntry) => {
    addTimelineEntry(entry);
    // Refresh follow-up after email sent (EmailComposer creates new follow-up via followUpConfig)
